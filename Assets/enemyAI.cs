...
public class enemyAI : MonoBehaviour
{
    ...
    void Update() {
        RaycastHit2D floorHit = Physics2D.Raycast(enemy.transform.position, new Vector2(0, -1f), 1f, environmentMask);
        RaycastHit2D playerHit = Physics2D.Raycast(enemy.transform.position, new Vector2(0, -1f), 1f, 1 << 9);
        ...
        if (health < 1) {enemy.SetActive(false);}

        switch (enemyState) { //actions
            case enemyStates.Idle:
